...
#         raise ValueError(f"'variants' field missing in {file}. Check if file is malformed or schema changed.")

#     df = df.withColumn("variant", explode("variants"))
#     df = df.withColumn("variant_id", col("variant.id"))
#     df = df.withColumn("variant_price", normalize_price_udf(col("variant.price")))
#     window_spec = Window.partitionBy("id").orderBy(col("created_at").cast("timestamp"))
#     df = df.withColumn("row_num", row_number().over(window_spec))

#     df = df.withColumn("reason",
#         when(col("id").isNull(), "Missing id")
#         .when(col("title").isNull(), "Missing title")
#         .when(col("variant_id").isNull(), "Missing variant id")
#         .when(col("variant_price").isNull(), "Invalid price")
#         .when(col("row_num") > 1, "Duplicate product id")
#         .otherwise(None)
#     )

#     valid_df = df.filter(col("reason").isNull()) \
#                 .drop("variants", "variant", "row_num") \
#                 .withColumn("price", col("variant_price")) \
#                 .drop("variant_price") \
#                 .withColumn("created_by", lit("batch_user")) \
#                 .withColumn("updated_at", lit(datetime.now().strftime('%Y-%m-%d %H:%M:%S')))

#     invalid_df = df.filter(col("reason").isNotNull()).select("id", "title", "variant_id", "variant_price", "reason")
...
def process_product(file):
    ...
    for dt_col in ["created_at", "updated_at", "published_at"]:
...
if "Contents" in response:
    for obj in response["Contents"]:
        ...
        if key.endswith(".json"):
            ...
            # if key.startswith("shopify/failed/products"):
            entity = "product"
            valid_df, invalid_df = process_product(file_path)
            # elif key.startswith("shopify/failed/locations"):
            #     entity = "location"
            #     valid_df, invalid_df = process_location(file_path)
            # elif key.startswith("shopify/failed/custom_collections"):
            #     entity = "customer"
